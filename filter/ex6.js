...
function filterOffensiveComments(comments, bannedWords) {
  const result = comments.filter( (x) => {
      if (x.toLowerCase().includes(bannedWords[0]) === false && x.toLowerCase().includes(bannedWords[1]) === false) {
          return x;
      }
  })
  return result;
...
console.log(filterOffensiveComments(["caca", "pipi", "popo"], ["pipi", "papa"]))
